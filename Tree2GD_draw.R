...
process_batch <- function(batch_list) {
  for(F in batch_list) {
    pdf(paste("./R_plotout/", F, ".ks.R.result.pdf", sep = ""), width = 15, height = 6)
    kaks = get(F)
    
    kaks_filtered = kaks[which(kaks$ks < 3), ]
    if (nrow(kaks_filtered) > 0) {
      kaks_p = ggplot(kaks_filtered, aes(x = ks, fill = Node)) +
        geom_histogram(binwidth = 0.05, alpha = 0.8, colour = "black", size = 0.3, position = "stack", show.legend = FALSE) +
        ...
        theme_classic() +
        theme(legend.key.size = unit(10, "pt"), legend.position = c(.9, .7), legend.key = element_blank(), legend.background = element_blank(), legend.text = element_text(size = 6), legend.title = element_text(size = 8))
      
      ks_filtered = kaks[which(kaks$ks < 3), ]
      if (nrow(ks_filtered) > 0) {
        ks_p = ggplot(ks_filtered, aes(x = ks, y = Node, fill = Node, alpha = rescale(sum, c(0, 1)))) +
    ...
    for(l in unique(kaks$level)) {
      subset_data = kaks[which(kaks$level == l & kaks$ks < 5), ]
      if (nrow(subset_data) > 0) {
        hist_data = table(cut(subset_data[, 6], breaks = seq(0, 5, by = 0.05)))
...
split_list_into_batches <- function(full_list, batch_size) {
  split(full_list, ceiling(seq_along(full_list) / batch_size))
}

batch_size <- 10  # Process 10 files per batch, adjust based on your memory

batches <- split_list_into_batches(kaks_list, batch_size)
...
p=ggtree(tree,size=rel(0.5),branch.length='none') %<+% summary %<+% dollop %<+% gdtype+geom_tippoint(color="#56556e",size=4)
...
p3=p2+
  geom_label(aes(x=branch, label=all), fill='grey',label.size = 0.2,size = 4,colour="blue")+
  geom_text(aes(x=branch, label=up), colour='green',size = 4,vjust=-1.5,check_overlap = T)+
  geom_text(aes(x=branch, label=down), colour='red',size = 4,vjust=+2.5)+
  scale_colour_manual(values =palette(rainbow(as.numeric(length(tree$tip.label)))),guide = FALSE)+
  geom_label(aes(label=describe),size = 4,vjust=+4,fill="#a5ecd7",label.padding = unit(0.03, "lines"),label.size=0.2,show.legend=FALSE)+
  #scale_fill_gradientn(colors = colorRampPalette(rev(brewer.pal(7,'RdYlBu')))(35),position="right",name="Color gradient")+
  geom_label(aes(label=GD),size = 4,fill= "#ff9a76",vjust=2,label.padding = unit(0.1, "lines"),label.size=0.2,show.legend=FALSE)+
  theme_classic()

p4=p3+geom_facet(data=all_kaks_result[which(all_kaks_result$ks<2&!is.na(all_kaks_result$ks)),],mapping = aes(x=`ks`,group=label,height = stat(density)),stat = "density",col="#e94560",size=0.5,scale = 15,rel_min_height = 0.001,alpha=15,geom=geom_density_ridges_gradient,panel = 'Ks Plot')+
geom_facet(data=all_kaks_result[which(all_kaks_result$`4dtv`<1&!is.na(all_kaks_result$`4dtv`)),],mapping = aes(x=`4dtv`,group=label,height = stat(density)),stat = "density",col="#e94560",size=0.5,scale = 15,rel_min_height = 0.001,alpha=15,geom=geom_density_ridges_gradient,panel = '4dtv Plot')
...
pdf("Tree2GD.result.pdf", width = (max(p$data$x)+1)*2.5, height = length(tree$tip.label)/2+5)
print(p5)
dev.off()
